...
result_df <- inner_join(g_df, book_attributes, by = c("from" = "id")) %>%
  ...
  filter(!is.na(from_first_genre)) %>% #filter any from empty genres
  filter(!is.na(to_first_genre)) #filter and to empty genres
...
result_df %>% filter(from_first_genre != to_first_genre) %>% nrow()
