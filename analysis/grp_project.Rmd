...
baseline_df <- baseline_df %>% 
  left_join(books_df %>% 
              select(id, rating, reviews, salesrank, 
                     first_genre, cleaned_genres, num_main_genres) %>% 
...
print(paste("Number of negative class in downsampled data:", sum(train_data$is_connected == 0)))
...
baseline_logit_model <- glm(is_connected ~ ., data = downsampled_data, 
...
merged_data <- downsampled_data_2 %>% 
  left_join(node_attributes_df %>% 
            ...
            by = c("V1" = "vertex_id")) %>% 
  left_join(node_attributes_df %>% select(vertex_id, degree, 
...
merged_test_data <- merged_test_data %>% 
  left_join(node_attributes_df %>% select(vertex_id, degree, closeness, 
