...
#calculate degree centrality and store as df
...
induced_subgraph(merged_total_network, node_fltr)
```
...
```
```{r}
deg_0302_book <- degree(g0302_book)
df_deg_0302_book <- as.data.frame(deg_0302_book) %>% mutate(bin = cut(.$deg_0302_book, breaks = 100, labels = FALSE))
df_deg_0302_book %>% left_join(book_data[, c("id", "salesrank")], by = c("deg_0302_book" = "id"))
```
