...
downsampled_data <- downSample(train_data[train_data$is_connected == TRUE, ], 
                               train_data[train_data$is_connected == FALSE, ], 
                               yname = "is_connected", 
                               list = FALSE,
                               p = 0.3)

# Combine the downsampled data
train_data <- rbind(downsampled_data, train_data[train_data$is_connected == TRUE,][seq_len(nrow(downsampled_data) * 0.7), ])
```
...
logistic_model <- glm(is_connected ~ ., data = train_data, family = binomial(link = "logit"))
