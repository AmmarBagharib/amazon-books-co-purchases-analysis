...
#view distribution of attribute length; those with 3 or less are all discontinued products
item_index %>% group_by(length) %>% summarize(n = n())

#save
#write.csv(item_index, "item_index.csv")
```


```{r preview item}
meta[item_index[item_index$length == 3, "start"][1]:item_index[item_index$length == 3, "stop"][1]]
```
